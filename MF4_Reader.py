...
class MDFViewerApp:
    def __init__(self, root):
        ...
        self.show_dots_checkbox = tk.Checkbutton(root, text="Show Dots on Graph", variable=self.show_dots_var)
        self.show_dots_checkbox.pack()
 
        self.use_raw_can_var = tk.BooleanVar(value=True)
        self.use_raw_can_checkbox = tk.Checkbutton(root, text="Decode Raw CAN using DBC", variable=self.use_raw_can_var)
    ...
    def load_decoded_channels_from_mf4(self):
        ...
        skip_keywords = ['CAN_DataFrame', 'DataBytes', 'Dir', 'BRS', 'ESI', 'IDE', 'DLC']
        ...
        try:
            ...
            for group_index, group in enumerate(self.mdf.groups):
                for channel_index, channel in enumerate(group.channels):
                    ...
                    unique_key = channel_name if channel_name not in seen else f"{channel_name} [g{group_index}]"
                    seen.add(channel_name)
 
                    try:
                        signal = self.mdf.get(channel_name, group=group_index, index=channel_index)
            ...
            messagebox.showinfo("Info", f"Found {len(self.signal_names)} decoded signals in MDF.")
                ...
                try:
                    ...
                    for signal_name, value in decoded.items():
                        ...
                        if signal_name not in self.decoded_signals:
                            self.decoded_signals[signal_name] = {'timestamps': [], 'values': [], 'unit': ''}
        ...
        if plot_mode == "same":
            ...
            for signal_name in selected_signals:
                ...
                if data is not None and len(data['timestamps']) > 0:
                    ...
                    plotted = True
            if not plotted:
                messagebox.showwarning("No Data", "No data found for the selected signals.")
                plt.close()
                return
            plt.title("Signals")
            plt.xlabel("Time [s]")
